...
cancer.df <- cancer.df %>%
  dplyr::select(-c(avg_deaths_per_year, avg_ann_count, pct_black, pct_asian, pct_other_race, binned_inc_point)) %>%
...
lasso1 <- cancer.df %>% dplyr::select(-c(region, study_quantile)) %>% lasso(., print = TRUE)
